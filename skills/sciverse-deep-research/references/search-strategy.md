...
## 证据压缩纪律（compress_evidence）

检索阶段读到的原始摘要、切片、网页正文数量大，长上下文下模型容易遗忘早期判断或把弱相关证据当成强证据。借鉴 LongSeeker 的 Context-ReAct 压缩机制：每处理 3–5 篇文献摘要后，必须写一次压缩块，把证据状态显式化，后续推理只基于压缩块而非原始摘要。
...
把压缩块追加进 **`.workflow/evidence_compress.md`**（固定文件名，不得并入 `search_log.md` 或仅在心里完成），格式固定：

```markdown
## 证据压缩块（视角 X / 第 N 批 / 关键词：...）

- 本批文献键：[@键1], [@键2], [@键3], ...
- 压缩摘要（1–2 句）：这批证据对本研究问题的共同贡献是什么？
- 证据状态：
  - [@键1]：✅ 已确认可用 — 直接支持 ...
  - [@键2]：⚠️ 待验证 — 摘要提及 ... 但需 `read_content` 对读确认
  - [@键3]：❌ 不满足要求 — 研究对象是 ... 非本主题核心
- 对本视角意图的修正：...（若有）
- 下一步动作：继续检索 / 补对读 / 放弃该方向
```

### 状态标记语义

- **✅ 已确认可用**：已做存在性核验，摘要/切片直接支持本主题某个子方向或 RQ。
- **⚠️ 待验证**：看起来相关，但关键 claim 需要 `read_content` 对读全文切片、或需要 `search_papers` 补题录字段才能确认。
- **❌ 不满足要求**：主题不相关、方法不匹配、无法核验存在性、或只是邻域工作；不纳入候选池，但保留记录作为"已排除"证据。

### 纪律

- 压缩块必须说明"哪些信息满足要求、哪些不满足"，不能只堆积极发现。
- 后续检索策略更新（updated_intent / missing_coverage）应基于压缩块状态汇总，而不是重新读原始摘要。
- 被标记 ❌ 的文献不再进入候选池；被标记 ⚠️ 的文献必须在进入台账前转为 ✅ 或 ❌。
- **未产出 evidence_compress.md 或块内无 ✅/⚠️/❌ 标记，视为阶段一未执行压缩纪律。**

## 检索意图演化（Retrieve → Reflect）

文献检索不是一次性 query→result 的匹配，而是**证据反馈下对用户意图的不断修正**。每完成一轮检索和压缩后，必须做一次显式反思（Reflect），再决定是否进入下一轮。灵感来自 PaSaMaster 的 Recursive Self-Evolving 检索：Navigator 根据已排序证据识别覆盖缺口、消歧约束、发现未充分探索的方向，并更新检索策略。

**每轮检索后的强制输出**（写入检索日志，作为下轮输入）：

```markdown
## 检索意图演化记录（第 t 轮）
- 本轮 query / 视角：...
- 关键发现：...（2-3 条，必须来自工具返回）
- updated_intent：...（对"到底在找什么"的更新表述）
- updated_checklist：...（本主题下候选论文应满足的检查项，可增删）
- missing_coverage：...（仍缺的方向/方法/争议/裁判型文献）
- evidence_gaps：...（已找到但证据不足、需进一步对读的条目）
- 是否触发补搜：是 / 否（预算内还有缺口则必须"是"）
```

**触发补搜的典型信号**：

- 某子方向核心文献 < 3 篇；
- 某个方法流派/关键争议在候选池中完全缺席；
- 裁判型文献定向检索未命中且该子主题存在明显方法边界；
- 多视角结果出现无法调和的矛盾，需要定向挖机制/条件差异文献；
- 时效探针显示近窗口内有重要工作但当前池未覆盖。

**fast-slow T-shaped 检索分工**（与 PaSaMaster 卡片对齐）：

- **fast（广度）**：并行执行多通道低成本扩展——语义直接召回、引文网络滚雪球（`list_paper_relations`）、相邻关键词变体、网页线索→论文验证。目标是快速扩大候选池，不做过深判断。
- **slow（深度）**：在 fast 结果上做意图消歧、约束澄清、检查清单更新、盲区识别，并据此生成 refined query 进入下一轮。slow 步骤只发生在策略层，不逐篇精读。
- 两者交替构成 T 形：fast 负责横向覆盖，slow 负责纵向精炼。
...
- **意图演化已记录：是/否（updated_intent / updated_checklist / missing_coverage）**
- 是否进入阶段二：是 / 继续补搜（理由）
```
...
6. **意图演化已记录**：至少完成一轮 Retrieve→Reflect，输出 `updated_intent`、`updated_checklist`、`missing_coverage`。若 `missing_coverage` 非空但预算已耗尽，允许进入阶段二，但必须在开放问题中显式列出这些缺口。
7. **显式判定**：用一句话给出 `has_enough_context_for_synthesis` 结论。`true` 才允许进阶段二；`false` 必须写清"缺哪块、下一步补搜关键词"。

> 本检查表把 research-depth.md 第 9 条"Planner 够了没闸门"和主 SKILL.md"滚雪球至饱和"落实成阶段切换前的强制出口，防止模型在上下文压力下"差不多就行"。
> 意图演化条款把 PaSaMaster "Recursive Self-Evolving" 的 prompt 层精华吸收进来：不训练专用 Ranker，只要求每轮检索后显式修正意图并留下证据。

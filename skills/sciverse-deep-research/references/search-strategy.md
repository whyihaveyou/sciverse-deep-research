...
文献检索不是一次性 query→result 的匹配，而是**证据反馈下对用户意图的不断修正**。每完成一轮检索后，必须做一次显式反思（Reflect），再决定是否进入下一轮。灵感来自 PaSaMaster 的 Recursive Self-Evolving 检索：Navigator 根据已排序证据识别覆盖缺口、消歧约束、发现未充分探索的方向，并更新检索策略。

# DFT 能算的物理量清单，及与晶格热导率（$\kappa_{L}$）的 Overlap

## 摘要

本文产出一份**可为用户直接使用的“DFT 能算的物理量”系统清单**，并按“是否与晶格热导率 $\kappa_{L}$ 相关”给每一项标注相关性等级，最后给出 **DFT 可算 ∧ 与 $\kappa_{L}$ 相关** 的 overlap 交集——这正是用户用来圈定“哪些 DFT 计算既可行、又喂给晶格导热研究”的清单。核心结论：DFT（含 DFPT）能算的物性横跨结构、电子、力学、热力学、输运五大谱系；其中与 $\kappa_{L}$ 直接相关的一阶量（决定 $\kappa_{L}$ 公式的输入）包括弹性张量/模量、声速、德拜温度、Grüneisen 参数、声子色散与群速度、三阶力常数（非谐）——这些全部可第一性计算；二阶级物理量（热容、热膨胀、声子寿命、$\kappa_{L}$ 本身）在其上一阶或一阶+散射计算之上得到。overlap 的核心：**你此前符号回归公式 / AFLOW 数据集里的几乎所有输入字段（B, G, $\rho$, V, N, $\gamma$, $\theta_{D,}$ $\theta_{ac,}$ Cv, $\alpha$）都是 DFT 可直接算的**，因此用你自己的 MLIP-DFT 工具产出一套“第一性真值特征集”是可行的——只需把当前依赖的 Slack 型 $\kappa_{L}$ 目标值换成 BTE 级 $\kappa_{L}$（见 RQ2）。

## 一、引言

目标很直接：既然要造替代 SLACK 的真值数据集，就得先知道 **DFT 到底能给我们哪些物理量**，再看其中**哪些和晶格导热（$\kappa_{L}$）有关**。本报告回答：

- **RQ1**：DFT（含 DFPT）能计算哪些物理量？把它们组织成一张可勾选的清单。
- **RQ2**：哪些 DFT 可算量直接决定/影响 $\kappa_{L}$（即与晶格热导相关）？挑出 overlap 交集。
...
**纳入/排除判据**：入选标准是「方法学代表性 + 子方向覆盖」，而非穷举——每个子方向（弹性 / 声子 / BTE 求解 / MLIP 加速 / quasiharmonic）至少 3 篇、横跨年份与工具族，保证清单一节归纳的是「共同方法」而非单篇特例；以开山基准 [2] 与工具包交叉基准 [3] 为锚点滚雪球追引前置技术链，直到连续两轮无新文献进入、每个子方向核心文献 ≥3 篇（饱和判据）。排除「纯应用、无方法学增量」的算例论文，以防清单被特例稀释。

## 三、DFT 能算的物理量：系统清单

DFT（含 DFPT 线性响应）第一性能力覆盖五大物理谱系。每一项给出：**它是什么、DFT 怎么算、常规可行度、与 $\kappa_{L}$ 的关联**——前三者界定“DFT 能不能给你这个量”，后者标出“这个量对晶格导热研究有没有用”。可行度标记：★★★=常规成熟；★★=需高阶力常数/后处理，可行但较重；★=前沿或高成本。
...
- 与 $\kappa_{L}$ 的关联（间接但全程打底）：晶格常数/体积是上面一切体积相关量的公共底座——它经 EOS/B 进入声速与德拜温度，进而喂给动理论 v 与 Slack 输入，是 3.3～3.4 各条公式链的「第一环」。
...
- 与 $\kappa_{L}$ 的关联：带隙宽窄影响电子-声子耦合，但对本征晶格热导的直接影响弱；主要通过“区分绝缘体/半导体/金属”（金属电子热导占主导，Slack 类公式不适用）间接相关。

### 3.3 力学/弹性（★★★）

弹性张量可从总能-应变或应力-应变拟合得到，是 DFT 最成熟的物性之一：

- **二阶弹性张量 $C_{ij}$、体模量 B、剪切模量 G、杨氏模量 E、泊松比 $\nu$**：单晶应变法常规可得（二阶常数 [4]，EOS/应变拟合），多晶的 B/G 用 VRH 平均（这和你们 AFLOW 数据集口径一致）。
- **弹性各向异性比**：单晶方向性信息，对“热导是否各向异性”有提示（多晶 VRH 会丢失）。
- **三阶/四阶弹性常数**：由总能-应变高阶拟合（应变需较大、数值敏感）[6]——三阶弹性常数与 Grüneisen 参数及非谐性直接相关，是连接“弹性 $\leftrightarrow$ 晶格导热”的桥梁。
- **声速 $v_{L/v}_{t/}$$v_{s}$**：由模量+密度导出，是动理论 $\kappa$=⅓Cv·v·l 中的 v，也是德拜温度的输入。
- 与 $\kappa_{L}$ 的关联（强）：B/G/声速/德拜温度是 Slack 型公式和你们符号回归公式的直接输入——其中德拜温度/声速喂给 Slack 输入（[7] 给 quasiharmonic Debye、[8] 给 Grüneisen-quasiharmonic 尺寸一致修正）。

### 3.4 声子与热力学（★★★）

- **声子色散、声子态密度、声子频率**：DFPT 或有限位移法（Phonopy 类工具）常规可得。DFPT 高通量需注意收敛与虚频陷阱 [5]。
- **德拜温度 $\theta_{D}$、声学德拜温度 $\theta_{ac}$**：由声速/声子谱导出；AFLOW/AGL 用 quasiharmonic Debye 给 [7]。
- **热容 Cv/Cp（声子贡献）**：由声子态密度直接积分。
- **Grüneisen 参数 $\gamma$**：声子频率对体积的依赖（quasiharmonic），是“非谐性”的核心度量，直接进 $\kappa_{L}$ 公式的非谐项（[7] 给 Debye 模型口径、[9] 给第一性实操口径）。
- **热膨胀系数 $\alpha$**：quasiharmonic 由 $\gamma$ 与热容得到。
- 与 $\kappa_{L}$ 的关联（极强）：声子谱是 BTE 热导的第一性输入；$\gamma$ 决定非谐散射强度；$\theta_{D}$ 决定高温区适用性。

### 3.5 输运（★★ 至 ★★★）

- **电子输运：电导 $\sigma$、Seebeck S、功率因子、电子热导 $\kappa_{el}$、ZT**：半经典 Boltzmann（BoltzTraP 类）常规可得 [1]。注意电子热导不能用常数 Lorenz 数粗暴估计（高 Seebeck/半导体时失效）[10]。
- **声子群速度 $v_{g}$**：由声子色散导数得到。
- **$\kappa_{L}$ 的第一性基准**：DFT+Boltzmann 无参数算 κL，Si/Ge 室温与实验 <5% 吻合 [2]——这是“DFT 能算$\kappa_{L}$”最硬的证据，也是你造库时对标的基准点。
- **三/四阶力常数 $\rightarrow$ 声子散射率/寿命 $\rightarrow$ 晶格热导 $\kappa_{L}$**：谐+三阶力常数 + BTE 求解，是 $\kappa_{L}$ 真值的黄金标准 [11, 12]。MLIP 可加速 [13, 14]。
- **载流子迁移率**：需形变势+散射近似，★★。
- 与 $\kappa_{L}$ 的关联（核心）：$\kappa_{L}$ 本身就是这一节的最高阶结果。

## 四、与晶格热导 $\kappa_{L}$ 的 Overlap：DFT 可算 ∧ 与 $\kappa_{L}$ 相关

晶格热导的三条公式链（动理论 $\kappa$=⅓Cv·v·l；Slack 型；BTE）决定了哪些量“既 DFT 可算、又决定 $\kappa_{L}$”。overlap 交集如下，按 $\kappa_{L}$ 公式的输入链标注：

### 4.1 一阶决定量（直接作为 $\kappa_{L}$ 公式的输入，DFT 全部可算）

| DFT 可算量 | 作用 | 可行度 |
|---|---|---|
| 体/剪切模量 B, G | 声速/德拜温度输入；符号回归公式特征 | ★★★ |
| 声速 $v_{s}$ | 动理论公式中的 v | ★★★ |
| 德拜温度 $\theta_{D,}$ 声学德拜温度 $\theta_{ac}$ | Slack/Wang/你们公式的输入 | ★★★ |
| Grüneisen 参数 $\gamma$ | 非谐项（$e^{-\gamma}$ / 1/$\gamma$²）核心输入 | ★★★ |
...
- 四声子/高阶散射：对声学声子强色散/本征非谐显著的体系（如纤锌矿、热电材料 SiGe/PbTe 类），三声子近似不足、强非谐材料需上四声子 [15]（★）
- **$\kappa_{L}$ 真值本身**：谐+三阶力量 $\rightarrow$ BTE 求解 [11, 12]（★★）

### 4.3 Overlap 结论（用户要的那个交集）

**DFT 可算 ∧ 与 $\kappa_{L}$ 直接相关**的物理量交集 = { B, G, $\rho$, V, N, $\gamma$, $\theta_{D,}$ $\theta_{ac,}$ Cv, $\alpha$, $v_{s,}$ 声子频率, 三阶力常数 }。其中前 10 项（B…$\alpha$）正是你 AFLOW 数据集里已有、但来自 Slack/AGL [7] 模型的字段——**用你自己的 MLIP-DFT 工具重算这些量，可得、但不绑定 SLACK、独立于 AGL [7] 的第一性特征集**（前提：MLIP 力常数需先经 §5 的 best-practice 校验后才算数）；而 $\kappa_{L}$ 本身应改用 BTE 级真值 [11, 12]，彻底摆脱 Slack。

## 五、对造真值数据集的直接启示

1. **先用 MLIP-DFT 批量重算 B, G, $\gamma$, $\theta_{D,}$ $\theta_{ac,}$ Cv, $\alpha$**（AFLOW 里这些是 Slack/AGL [7] 输出，你重算即得独立真值特征集；该高通量重算路径可由 MLIP 通用势加速 [16]）。
2. **$\kappa_{L}$ 入库值用 BTE 级**（谐+三阶+BTE 或 MLP-MD 交叉校验），不再用 Slack [11, 14]。基准对照：各方法在 Si/Ge 上与实验结果应 <5% 内 [2]（该 <5% 精度目前仅在 Si/Ge 等简谐居中的基准体系上得到验证，且含体散射假设；对强非谐/低对称体系需逐体系核对）。
...
- **RQ1**：DFT 能算结构、电子、力学、声子热力学、输运五大类物性，主流量（模量/声速/德拜/$\gamma$/声子/热电）成熟可算 [6, 4, 5, 1]。
- **RQ2**：与 $\kappa_{L}$ 相关的 DFT 可算交集 = {B, G, $\rho$, V, N, $\gamma$, $\theta_{D,}$ $\theta_{ac,}$ Cv, $\alpha$, $v_{s,}$ 声子频率, 三阶力常数, $\kappa_{L}$(BTE)} [11, 12]——这些正是你们 MLIP-DFT 工具能产出的第一性特征集，可支撑造替代 SLACK 的真值数据集。

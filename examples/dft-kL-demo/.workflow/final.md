...
- 与 κ_L 的关联（间接但全程打底）：晶格常数/体积是上面一切体积相关量的公共底座——它经 EOS/B 进入声速与德拜温度，进而喂给动理论 v 与 Slack 输入，是 3.3～3.4 各条公式链的「第一环」。
...
- **二阶弹性张量 C_ij、体模量 B、剪切模量 G、杨氏模量 E、泊松比 ν**：单晶应变法常规可得（二阶常数 [4]，EOS/应变拟合），多晶的 B/G 用 VRH 平均（这和你们 AFLOW 数据集口径一致）。
...
- 与 κ_L 的关联（强）：B/G/声速/德拜温度是 Slack 型公式和你们符号回归公式的直接输入——其中德拜温度/声速喂给 Slack 输入（[7] 给 quasiharmonic Debye、[8] 给 Grüneisen-quasiharmonic 尺寸一致修正）。
...
- **Grüneisen 参数 γ**：声子频率对体积的依赖（quasiharmonic），是"非谐性"的核心度量，直接进 κ_L 公式的非谐项（[7] 给 Debye 模型口径、[9] 给第一性实操口径）。
...
**DFT 可算 ∧ 与 κ_L 直接相关**的物理量交集 = { B, G, ρ, V, N, γ, θ_D, θ_ac, Cv, α, v_s, 声子频率, 三阶力常数 }。其中前 10 项（B…α）正是你 AFLOW 数据集里已有、但来自 Slack/AGL [7] 模型的字段——**用你自己的 MLIP-DFT 工具重算这些量，可得、但不绑定 SLACK、独立于 AGL [7] 的第一性特征集**（前提：MLIP 力常数需先经 §5 的 best-practice 校验后才算数）；而 κ_L 本身应改用 BTE 级真值 [11, 12]，彻底摆脱 Slack。

## 五、对造真值数据集的直接启示

1. **先用 MLIP-DFT 批量重算 B, G, γ, θ_D, θ_ac, Cv, α**（AFLOW 里这些是 Slack/AGL [7] 输出，你重算即得独立真值特征集；该高通量重算路径可由 MLIP 通用势加速 [16]）。
2. **κ_L 入库值用 BTE 级**（谐+三阶+BTE 或 MLP-MD 交叉校验），不再用 Slack [11, 14]。基准对照：各方法在 Si/Ge 上与实验结果应 <5% 内 [2]（该 <5% 精度目前仅在 Si/Ge 等简谐居中的基准体系上得到验证，且含体散射假设；对强非谐/低对称体系需逐体系核对）。
